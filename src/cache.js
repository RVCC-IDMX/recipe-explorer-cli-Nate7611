...
export async function initializeCache() {
  ...
  try {
    ...
    try {
      ...
      console.error('Error initializing cache:', writeError);
    }
  }
}
...
export async function getFromCache(key) {
  ...
  try {
    ...
    if (cache[key]) {
      const { timestamp, data } = cache[key];
...
export async function saveToCache(key, data) {
  ...
  try {
    ...
    cache[key] = { timestamp: Date.now(), value: data };
...
export async function clearExpiredCache() {
  ...
  try {
    ...
    const fileData = await fs.readFile(CACHE_FILE);
    const cache = JSON.parse(fileData);
    const currentTime = Date.now();
    let removedCount = 0;

    for (const key in cache) {
      if (currentTime - cache[key].timestamp >= CACHE_DURATION) {
        delete cache[key];
        removedCount++;
      }
    }
  ...
  try {
    ...
    if (!forceRefresh) {
      try {
        ...
        console.error('Error accessing expired cache:', fallbackError);
      }
    }
