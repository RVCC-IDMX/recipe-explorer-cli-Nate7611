...
async function searchRecipes() {
  ...
  try {
    ...
    if (recipes && recipes.length > 0) {
      const viewDetails = readlineSync.keyInYN('Would you like to details for a specific recipe?');
...
async function viewRecipeDetails(recipeId) {
  ...
  try {
    ...
    const recipe = await cache.getCachedOrFetch(cacheKey, () => { return api.getMealById(recipeId) });
    ...
    if (isFavorite) {
      const remove = readlineSync.keyInYNStrict('Would you like to remove this item from favorites?');
      if (remove) {
        await favorites.removeFavorite(recipeId);
        console.log('Recipe removed from favorites!');
      }
    } else {
      const add = readlineSync.keyInYNStrict('This recipe is not in your favorites. Would you like to add it?');
      if (add) {
...
async function exploreByFirstLetter() {
  ...
  try {
    ...
    const recipes = await cache.getCachedOrFetch(cacheKey, () => { return api.searchMealsByFirstLetter(uniqueLetters) });

    console.log(utils.formatRecipeList(recipes));

    if (recipes && recipes.length > 0) {
      const viewDetails = readlineSync.keyInYN('Would you like to view details for a specific recipe?')
...
async function searchByIngredient() {
  ...
  try {
    ...
    const recipes = await cache.getCachedOrFetch(cacheKey, () => { return api.getMealsByIngredient(ingredient) });
...
async function discoverRandom() {
  ...
  try {
    ...
    if (isFavorite) {
      const remove = readlineSync.keyInYNStrict('Would you like to remove this item from favorites?');
      if (remove) {
        await favorites.removeFavorite(randomRecipe);
        console.log('Recipe removed from favorites!');
      }
    } else {
      const add = readlineSync.keyInYNStrict('This recipe is not in your favorites. Would you like to add it?');
