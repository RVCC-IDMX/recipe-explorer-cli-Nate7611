...
export async function searchMealsByName(query) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`searchMealsByName: HTTP ${response.status} - ${response.statusText}`);
...
export async function getMealById(id, attempts = 2) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`getMealById: HTTP ${response.status} - ${response.statusText}`);

    const data = await response.json();
    return data.meals ? data.meals[0] : null;
  } catch (error) {
    console.error(`Error getting meal by ID (ID: ${id}):`, error);
  ...
  try {
    const fetchPromises = letters.map(letter =>
      fetch(`${BASE_URL}/search.php?f=${letter.charAt(0)}`)
        .then(response => {
          if (!response.ok) {
            throw new Error(`searchMealsByFirstLetter: HTTP ${response.status} - ${response.statusText}`);
          }
    ...
    console.error("Error in searchMealsByFirstLetter:", error);
    return [];
  }
}
...
export async function getMealsByIngredient(ingredient, timeoutMs = 5000) {
  ...
  const fetchPromise = (async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`getMealsByIngredient: HTTP ${response.status} - ${response.statusText}`);
      }

      const data = await response.json();
      return data.meals || [];
    } catch (error) {
      console.error(`getMealsByIngredient(ingredient: "${ingredient}") - ${error.message}`);
      return [];
    }
  })();
...
export async function getRelatedRecipes(recipe, limit = 3) {
  ...
  if (!recipe || !recipe.strCategory) {
    console.error("getRelatedRecipes - Invalid recipe object provided");
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`getRelatedRecipes: HTTP ${response.status} - ${response.statusText}`);
    }

    const data = await response.json();
    if (!data.meals) return [];

    const filteredRecipes = data.meals.filter(meal => meal.idMeal !== recipe.idMeal).slice(0, limit);

    return filteredRecipes;
  } catch (error) {
    console.error(`getRelatedRecipes(category: "${recipe.strCategory}") - ${error.message}`);
    return [];
  }
}
...
export async function getRandomMeal() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`getRandomMeal: HTTP ${response.status} - ${response.statusText}`);
    }

    const data = await response.json();
    return data.meals ? data.meals[0] : null;
  } catch (error) {
    console.error(`getRandomMeal - ${error.message}`);
    return null;
  }
}
